...
| 组件 | 最低配置               | 推荐配置                               |
| ---- | ---------------------- | -------------------------------------- |
| GPU  | 16 GB 显存（第一模块） | NVIDIA RTX PRO 6000 Blackwell（96 GB） |
| CPU  | 8 核                   | 32+ 核（并行突变扫描）                 |
| 内存 | 32 GB                  | 96 GB                                  |
...
| NVIDIA 驱动 | ≥550.x（Blackwell sm_120） | RTX PRO 6000 需要             |
...
| 库              | 版本           | 所属环境      | 用途                    |
| --------------- | -------------- | ------------- | ----------------------- |
| PyRosetta       | 2026.19        | pyrose        | 对接、打a a分、突变扫描 |
...
wget -P RFdiffusion/models/ \
  http://files.ipd.uw.edu/pub/RFdiffusion/6f5902ac237024bdd0c176cb93063dc6/Complex_base_ckpt.pt
```
